...
if python -m src.drug_combs.clinical_trials_combinations --aact_params_file_path input_data/aact_credentials.json "data/final_schema/${now}/aact_combs.csv"; then
...
if python -m src.drug_combs.add_identifier_to_df --as_str_array True "data/final_schema/${now}/aact_combs.csv" selected_name identifiers_entity "$aact_with_identifiers_path"; then
...
if python -m src.drug_combs.schema_transforming "$aact_with_identifiers_path" data/final_schema/"${now}" input_data/dbid_to_compound.csv; then
...
if python -m src.drug_combs.orange_book input_data/orange_book "data/final_schema/${now}/orangebook_combs_df.csv"; then
...
if python -m src.drug_combs.create_unnormalized_combs_db data/final_schema/"${now}" data/final_schema/"${now}"; then
